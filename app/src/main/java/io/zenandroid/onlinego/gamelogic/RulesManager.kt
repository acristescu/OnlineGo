...
object RulesManager {
  ...
  private fun getCacheKey(
    game: Game,
    actualMoves: List<Cell>,
    computeTerritory: Boolean
  ): CacheKey {
    return CacheKey(
      width = game.width,
      height = game.height,
  ...
  fun replay(
    ...
    val actualMoves = when {
      variation == null || limit < variation.rootMoveNo -> game.moves?.take(limit)
        ?.map { Cell(it.x, it.y) }

      else -> game.moves?.take(variation.rootMoveNo)?.map { Cell(it.x, it.y) }
        ?.plus(variation.moves.take(limit - variation.rootMoveNo))
    } ?: emptyList()
  ...
  fun buildPos(
    ...
    val isLiveStone: (Cell) -> Boolean =
      { (whiteStones.contains(it) || blackStones.contains(it)) && !removedCells.contains(it) }
    val isMarkedDame: (Cell) -> Boolean =
      { removedCells.contains(it) && !whiteStones.contains(it) && !blackStones.contains(it) }
    //
        ...
        (0 until boardHeight)
          ...
          .forEach {
            ...
            while (toVisit.isNotEmpty() && !(foundBlack && foundWhite)) {
              val p = toVisit.removeAt(toVisit.lastIndex)
              visited.add(p)
  ...
  fun scorePositionPartial(pos: Position, game: Game): Pair<Score, Score> {
    ...
    val whiteScore = Score(
      ...
      total = (whiteHandicap ?: 0) + (pos.komi ?: 0f) + (whitePrisoners ?: 0) + (whiteTerritory
        ?: 0),
    )
    val blackPrisoners =
