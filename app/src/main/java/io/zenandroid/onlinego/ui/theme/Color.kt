package io.zenandroid.onlinego.ui.theme

import androidx.compose.ui.graphics.Color

val mainBlue = Color(0xFF79B3E4)
val nightBlue = Color(0xFF389FFF)
val darkBlue = Color(0xFF2588C9)
val salmon = Color(0xFFED7861)
val background = Color(0xFFF2F4F7)
val lightOnSurface = Color(0xFF212121)
val lightOnBackground = Color(0xFF9B9B9B)
val lightOnPrimary = Color.White

val brown = Color(0xFF443741)
val brownMedium = Color(0xFF6B5569)
val brownLite = Color(0xFF867484)
val accent = Color(0xFFFEDF47)

val nightSurface = Color(0xFF16151A)
val nightOnSurface = Color(0xFFF0F0F0)
val nightBackground = Color(0xFF000000)
val nightOnBackground = Color(0xFF9B9B9B)
val nightOnPrimary = Color.White